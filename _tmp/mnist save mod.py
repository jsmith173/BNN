...
gen_deterministic_model = 0; eval_deterministic_model = 1
gen_prob_model = 0; eval_prob_model = 0
...
def get_deterministic_model(input_shape):
...
def get_probabilistic_model(input_shape):
    """
...
if gen_deterministic_model == 1:
    ...
    deterministic_model = get_deterministic_model(input_shape=input_shape)
    deterministic_model.save('deterministic_mnist.keras')  
    deterministic_model.compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
...
if gen_prob_model == 1:
    tf.random.set_seed(0)
    probabilistic_model = get_probabilistic_model(input_shape=input_shape)
    probabilistic_model.save('prob_mnist.keras')  
    probabilistic_model.compile(loss=nll, optimizer="adam", metrics=["accuracy"])
    probabilistic_model.summary()    
    probabilistic_model.fit(x_train, y_train, batch_size=128, epochs=5, validation_split=0.1)
else:
    probabilistic_model = tf.keras.models.load_model('prob_mnist.keras', custom_objects={'loss': nll})
